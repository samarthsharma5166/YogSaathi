...
import { class_reminder, class_reminder_free_yoga_for_all, confirmation_regn, days_yoga_trial_intimation_hindi, festival_greetings, festival_greetings_christmas_new_year, free_online_yoga_trial_reminder, giftwellness_yogsaathi, inputs, join_session__mark_attendance, onetoone_yoga_support, online_free_yoga_trial__joining_details, opi, orientation_program__new, regularity_key_hindi, retreat_info_brochure, session_info, session_particulars, session_reminder, session_reminder__orientation_for_free_trial, session_schedule_notification, share_wellness_14_days_of_free_yoga, subscription_invitation, subscription_offer_, subscription_plan_new_year_offer, template_session_20260627022218, trial_expiry_notification, vijayadashami_greetings, vijaydashmi_greetings_and_referrals, weekly_attendance_status__yogsaathi_sessions, world_meditation_day_greetings, yoga_class_time_details_as_per_ist, yoga_offer_reminder, yoga_session_info_f, yoga_subscription_offer, yoga_trail_intimation_, yoga_training_1ram, yoga_training_2, yoga_trial_enrolment, yoga_trial_midway_update__reminder, yoga_trial_participation_reminder, yogsaathi_class_attendance_reminder, yogsaathi_communication_channels, yogsaathi_contact_detail, yogsaathi_group_access_update, yogsaathi_payment_link_share, yogsaathi_training_brochure_share, your_weekly_yoga_schedule__access_details } from '../utils/messages.js';
...
export const hourlyJob = new CronJob('*/10 * * * *', async () => {
    ...
    scheduledMessages.forEach(async (message) => {
        ...
        if (message.templateName === "yogsaathi_class_attendance_reminder"){
            ...
            for (const user of users) {
                try {
                    await yogsaathi_class_attendance_reminder(user.phoneNumber, user.name || "Yogi");
                    await new Promise(resolve => setTimeout(resolve, 100));
                } catch (error) {
                    console.error(`Error sending yogsaathi_class_attendance_reminder to ${user.phoneNumber}:`, error.message);
                }
