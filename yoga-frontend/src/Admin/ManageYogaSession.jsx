...
import {
  ...
  getYogaSessionRegistrations,
  downloadYogaSessionRegistrations,
} from "../services/api";
import { Calendar, Clock, DollarSign, Users, Award, ShieldAlert, Sparkles, Filter, Download } from "lucide-react";

export default function ManageYogaSession() {
  ...
  const fetchRegistrations = async () => {
    setLoadingRegs(true);
    try {
      const res = await getYogaSessionRegistrations();
  ...
  const handleDownloadExcel = async () => {
    try {
      const response = await downloadYogaSessionRegistrations();
      const blob = new Blob([response.data], { type: "text/csv" });
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement("a");
      link.href = url;
      link.setAttribute("download", "yoga-session-registrations.csv");
  ...
  const handleUpdateConfig = async (e) => {
    ...
    try {
      const res = await updateYogaSessionConfig({
        price: Number(priceInput),
        slotsLeft: Number(slotsInput),
      });
      setConfig(res.data);
      toast.success("Yoga session masterclass settings updated successfully!");
    } catch (err) {
      console.error(err);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight flex items-center gap-2">
            <Sparkles className="w-6 h-6 text-green-600" /> Yoga Session Management
          </h1>
          <p className="text-gray-500 text-sm mt-1">
            Configure pricing, manage slots, and track Lifestyle Disorders masterclass registrations.
          </p>
        </div>
        <button
          onClick={() => {
