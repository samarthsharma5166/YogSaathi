...
const YogaSessionLeads = () => {
  ...
  const fetchLeads = async () => {
    try {
      setLoading(true);
      const response = await getYogaSessionLeads(page, 20, searchQuery);
      ...
      console.error("Error fetching yoga session leads:", error);
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      setUploading(true);
      const response = await uploadYogaSessionLeads(formData);
      if (response.data && response.data.success) {
  ...
  const handleDeleteLead = async (id) => {
    ...
    try {
      const response = await deleteYogaSessionLead(id);
      if (response.data && response.data.success) {
  ...
  const handleClearAll = async () => {
    if (
      !window.confirm(
        "WARNING: This will permanently delete ALL uploaded yoga session leads. This action cannot be undone. Are you sure?"
      )
    ) {
      return;
    }
    ...
    try {
      const response = await clearYogaSessionLeads();
      if (response.data && response.data.success) {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Yoga Session Leads</h1>
          ...
          <div className="flex flex-col justify-center items-center py-20 text-center px-4">
            ...
            <p className="text-gray-500 text-sm mt-1 max-w-sm">
              {searchQuery
                ...
                : "Get started by importing your yoga session Google Form responses above."}
            </p>
          </div>
        ) : (
...
export default YogaSessionLeads;
